...
// Certains symboles sont manquants dans uefi.h
...
static int IsKBHit();
static int ReadKBByte(); // This is the functionality we want to override in the emulator.
...
restart: {
  FILE *f = fopen(image_file_name, "r");
  if (!f) {
  ...
  core->regs[11] =
      dtb_ptr ? (dtb_ptr + MINIRV32_RAM_IMAGE_OFFSET)
...
static int ReadKBByte() {
  char rxchar = last_key;
  last_key = 0;

  return rxchar ? rxchar : -1;
}
...
static int64_t SimpleReadNumberInt(const char *number, int64_t defaultNumber) {
  ...
  char *endptr;
  uint64_t ret = strtol(number, &endptr, radix);
